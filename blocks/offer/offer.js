...
export async function decoratePostLogin(block, tag) {
  const aempublishurl = getAEMPublish();
  const aemauthorurl = getAEMAuthor();
  const persistedquery = '/graphql/execute.json/securbank/OfferByTag';

  const url = window.location && window.location.origin && window.location.origin.includes('author')
    ? `${aemauthorurl}${persistedquery};tag=${tag};ts=${Math.random() * 1000}`
    : `${aempublishurl}${persistedquery};tag=${tag};ts=${Math.random() * 1000}`;
  const options = { credentials: 'include' };
